...
fn color(byte: u8) -> Color {
    // White for Snake, Black for BG, varying colours
    // for the apple.
    match byte {
...
fn handle_user_input(cpu: &mut CPU, event_pump: &mut EventPump) {
    // the address 0xFF stores the lates user input
...
fn main() {
    ...
    //load the game
    let nes_file_data: Vec<u8> = std::fs::read("Alter_Ego.nes").unwrap();
